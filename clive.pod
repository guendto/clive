...
Write extracted video file to I<file>.
...
Causes B<clive> to log into Youtube as I<username>. This is done I<before>
extraction, even if the batch does not contain any Youtube video links.

=item B<-t --youtube-pass=>I<password>

Use I<password> for Youtube login.  If undefined, B<clive> prompts for it.
...
B<clive> forks the I<command> and continues with the transfer. Note that
B<clive> will not continue to download another file before the child process
terminates.
...
Note that B<clive> does nothing to check if there is enough data buffered
before forking the stream process. If the transfer rate drops significantly
after starting the process and it runs out of data, B<clive> does nothing
to fix this.
...
available. For example the high-definition (HD) formats (35, 22) are
...
Written by Toni Gundogdu <legatvs@gmail.com>. For a list of contributors,
please see the AUTHORS and the CHANGES files.
